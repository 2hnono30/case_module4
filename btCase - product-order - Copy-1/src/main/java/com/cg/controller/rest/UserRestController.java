...
public class UserRestController {
    ...
    @GetMapping("/deleAccList")
    public ResponseEntity<?> getDeleAccList() {

        List<UserDTO> userDTOS = new ArrayList<>();

        List<User> users = userService.findUserByDeletedIsTrue();
...
//    }
    ...
    public ResponseEntity<?> delete(@PathVariable Long userId) {
        ...
        if (userOptional.get().getRole().getId() == 1){
            throw  new SecurityException("Ai lại đi xóa acc ADMIN ???");
        }
        try {
            User user = userOptional.get();
            user.setDeleted(true);
            user.setActive(true);
            User newUser = userService.deleteRecoveUser(user);
            UserDTO userDTO =newUser.toUserDTO1(newUser.getLocationRegion().toLocationRegionDTO());
    ...
    public ResponseEntity<?> recover(@PathVariable Long userId) {
        Optional<User> userOptional = userService.findById(userId);
        if (!userOptional.isPresent()) {
            throw new DataInputException("Not User");
        }
